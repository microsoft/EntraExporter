function Export-Entra
{
    ...
    foreach ($schemaItem in $requestedExportSchema)
    {
        ...
        {
            Write-Verbose "Skipping '$($schemaItem.Path)' as it doesn't support '$schemaScopeType'"
            continue
        }
        ...
        while ($script:childrenToProcess.Count -gt 0)
        {
            ...
            foreach ($childGroup in $currentLevel)
            {
                ...
                Write-Verbose "Processing children for '$($childGroup.ParentPath)' ($($allParentIds.Count) parents)"
